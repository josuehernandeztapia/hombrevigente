# GHK-Cu (Copper Peptide GHK)

**Categoría**: Péptido / cosmética regenerativa  
**Avenida HV**: 1 (tópico oral suplemento si legal) / 2 (inyectable)  
**Evidencia predominante**: E2–E3 (tópico)  
**Tags**: GHK-Cu, colágeno, piel, wound healing, epigenética

---

## 📋 Definición

**GHK** (Gly-His-Lys) es un tripeptido unido a cobre (GHK-Cu) con roles en **remodelación tisular**, síntesis de colágeno y modulación génica en fibroblastos.

- Pickart L, Margolina A. Regenerative and protective actions of the GHK-Cu peptide. Int J Mol Sci. 2018. PMID: 30103415 (E2 review)
- Pickart L, et al. GHK peptide as a natural modulator of multiple cellular pathways. BioMed Res Int. 2015. PMID: 26064896 (E2)

---

## 🔬 Mecanismo de acción

1. **Síntesis colágeno y elastina** en fibroblastos humanos in vitro (E2).
2. **Metaloproteinasas (MMP)** modulación — remodelación matriz extracelular (E2).
3. **Expresión génica:** "reset" parcial de perfil génico en fibroblastos hacia patrón más joven en cultivo (E2) — base del claim epigenético.
4. **Angiogénesis y antiinflamatorio** en heridas (E2).
5. **Antioxidante** vía cobre/Cu-Zn SOD pathway (E2).

---

## ✅ Evidencia por vía

| Vía | Evidencia | Tier |
|-----|-----------|------|
| Tópico (cremas, serums) | Mejora fotoenvejecimiento, firmeza en estudios pequeños | E3 |
| Inyectable subcutáneo | Uso biohacking MX; literatura inyectable humana limitada | E2 comunidad |
| Oral | Biodisponibilidad y eficacia debatidas | E2 débil |

**Conexión estética HV:** puente entre longevidad y "glow" — colágeno, textura, recuperación post-procedimiento (educación).

---

## 🏷️ Uso HV

**Avenida 1 (tópico / educación):**
> "GHK-Cu tiene evidencia en **cuidado dérmico y remodelación de matriz** (E2–E3). En optimización de **piel y recuperación**, se usa tópico o en stacks orales según producto regulado como cosmético/suplemento."

**Avenida 2 (inyectable):**
> "Inyecciones de GHK-Cu son práctica de biohacking con base preclínica (E2); requieren médico y vía legal."

---

## 📊 Biomarcadores / señales

- Evaluación visual RGB (Haut.AI) — textura, manchas (E3 tech)
- No biomarcador sanguíneo específico GHK

---

## ⚠️ Consideraciones

- **Cobre:** exceso sistémico teórico — hepatotoxicidad cobre en enfermedad de Wilson (E4); no aplicable a tópico usual.
- Calidad inyectable: crítica.

---

## 📖 FAQ

**¿GHK-Cu "rejuvenece" epigenéticamente?**  
Solo en fibroblastos cultivados (E2). Lenguaje: "modulación de expresión génica en modelos celulares".