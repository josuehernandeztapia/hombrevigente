...
class TestRagRetrievalLocal(unittest.TestCase):
    ...
    def test_lexical_retrieval_returns_chunks(self):
        ...
        try:
            from rag_retrieval_local import rag_query_local, _kb_path
        except Exception as e:  # pragma: no cover
            self.skipTest(f"rag_retrieval_local import failed: {e}")
        # The embeddings index (~17MB) is generated by kb_pipeline and deployed
        # out-of-band, not committed. Skip when absent (e.g. CI) rather than fail.
        if not _kb_path().exists():
            self.skipTest("knowledge_base/embeddings_local.json not present in this environment")
